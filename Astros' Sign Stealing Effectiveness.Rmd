...
player_stats_clean <- player_stats %>%
  mutate(
    ...
    Player = case_when(             
      grepl("altuve", Player) ~ "Jose Altuve",
      grepl("correa", Player) ~ "Carlos Correa",
      grepl("bregman", Player) ~ "Alex Bregman",
      grepl("gonzalez", Player) ~ "Marwin Gonzalez",
      grepl("gurriel", Player) ~ "Yuli Gurriel",
      TRUE ~ Player
    )
  )
...
player_stats_with_bangs <- player_stats_clean %>%
  left_join(
    bangs_clean %>% select(game_date, batter, has_bangs, bangs),
    by = c("game_date" = "game_date", "Player" = "batter")
  )
player_stats_with_bangs <- player_stats_with_bangs %>%
  mutate(
    batting_avg = H / AB
  )
...
game_results_clean <- game_results %>%
  mutate(
    Date_clean = str_remove(Date, "^[A-Za-z]+\\s+"),  # Now "Apr 3"
    Date_full = paste(Date_clean, "2017"),
    game_date = mdy(Date_full),     
    Opp = str_squish(str_trim(Opp))
  ) %>%
...
bangs_by_score <- bangs_clean %>%
  mutate(score_diff = final_home_runs - final_away_runs) %>%
  group_by(score_diff) %>%
  summarize(total_bangs = n())
...
ggplot(game_bangs_summary, aes(x = total_bangs, y = R)) +
...
correlation <- cor.test(
  ~ total_bangs + R,
  data = game_bangs_summary %>% filter(!is.na(total_bangs), !is.na(R))
)
...
performance_comparison <- player_stats_with_bangs %>%
  filter(has_bangs %in% c("y", "n")) %>%
  group_by(has_bangs) %>%
  summarize(
...
ggplot(performance_tidy, aes(x = metric, y = value, fill = has_bangs)) +
  geom_col(position = "dodge") +
  scale_fill_manual(values = c("n" = "#002D62", "y" = "#EB6E1F"), 
                    labels = c("y" = "With Bang", "n" = "Without Bang")) +
  labs(
...
player_difference <- player_stats_with_bangs %>%
  group_by(Player, has_bangs) %>%
  summarize(
    at_bats = sum(AB),
    hits = sum(H),
    batting_avg = hits / at_bats,
  ...
  pivot_wider(
    names_from = has_bangs,
    values_from = c(at_bats, hits, batting_avg),
    names_glue = "{.value}_{has_bangs}"
  ) %>%
  mutate(
    avg_difference = batting_avg_y - batting_avg_n,
    sufficient_sample = (at_bats_y >= 20 & at_bats_n >= 20)
  ) %>%
...
ggplot(top_improvers, aes(x = reorder(Player, avg_difference), y = avg_difference)) +
  geom_col(fill = "#002D62") +
...
t_test_data <- player_stats_with_bangs %>%
  filter(has_bangs %in% c("y", "n"), AB > 0) %>%
  mutate(batting_avg = H / AB)


t_test_result <- t.test(batting_avg ~ has_bangs, data = t_test_data)


t_test_table <- tibble(
  Metric = "Batting Average",
  With_Bang = mean(t_test_data$batting_avg[t_test_data$has_bangs == "y"], na.rm = TRUE),
  Without_Bang = mean(t_test_data$batting_avg[t_test_data$has_bangs == "n"], na.rm = TRUE),
...
bangs_tagged <- bangs_clean %>%
  filter(!is.na(description)) %>%
  mutate(
    is_strike = description %in% c("Called Strike", "Swinging Strike", "Foul", "Foul Tip"),
    is_ball = description == "Ball"
  )

bang_heatmap <- bangs_tagged %>%
  group_by(is_ball, is_strike) %>%
  summarize(
    total_bangs = n(),
